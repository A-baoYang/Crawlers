...
root_dir = "/home/abao.yang@cathayholdings.com.tw"
data_rootpath = "forResearch/data/gcs/Crawling/"
freeproxy_filepath = os.path.join(
    root_dir, data_rootpath, "valid_ips.csv")
smartproxy_filepath = os.path.join(
    root_dir, data_rootpath, "smartproxy_proxies.csv")
smartproxy_datacenter_filepath = os.path.join(
    root_dir, data_rootpath, 
    "smartproxy_test_datacenter_proxies.csv")
listed_company_filepath = os.path.join(
    root_dir, data_rootpath, 
    "Financial/data/twse_securities_ids.csv")
    ...
    while (changedIp < maxtimes_changeIp) and (success == 0):
        ...
        while (retry < maxtimes_retry) and (success == 0):
            ...
            try:
                ...
                assert soup.title
                success = 1
                logger.info(f"Success: {soup.title}")
