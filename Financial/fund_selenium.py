...
fund_types = [
    ...
    for item in driver.find_elements_by_xpath('//div[@class="InternalSearch"]//a')
]
print(fund_types[:5])
...
for item in tqdm(fund_types):
    ...
    fund_company = [
        (item.text, item.get_attribute("href"))
        for item in driver.find_elements_by_xpath('//table[@id="oMainTable"]//td/a')
    ...
    try:
        ratio_content = WebDriverWait(driver, 3).until(
            EC.presence_of_element_located(
                (By.XPATH, '//table[@class="t04"]//table[@class="t01"]/tbody')
            )
        )
...
for k in fund_ratio.keys():
    ...
    cl = np.unique(
        [
            char.strip()
            for char in re.findall(r"[\u4e00-\u9fff- a-zA-Z]+|91APP", content)
            if (char.strip() != "")
            and (char.strip() != "-")
            and (char.strip() != "KY")
            and (char.strip() != "-KY")
...
print(len(collect))
pd.DataFrame(
    collect, columns=["fund_id", "rel", "target_name"]
).drop_duplicates().to_csv("fund_holding_relation.csv", index=False)
